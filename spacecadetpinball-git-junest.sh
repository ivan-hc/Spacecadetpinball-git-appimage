...
BIN="SpaceCadetPinball" #CHANGE THIS IF THE NAME OF THE BINARY IS DIFFERENT FROM "$APP" (for example, the binary of "obs-studio" is "obs")
DEPENDENCES="ca-certificates alsa-lib flac fluidsynth glibc hidapi jack libdecor libgl libmodplug libpulse libusb libvorbis libx11 libxcursor libxext libxrender mpg123 opusfile pipewire sdl2 sdl2_mixer" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCIES ARE NEEDED
...
_post_installation_processes() {
	printf "\n◆ User's processes: \n\n"
	wget -q https://images.pling.com/cache/400x400/img/00/00/57/46/34/1855605/icon-192x192.png -O AppDir/icon.ico.png
	sed -i 's#/usr/lib/spacecadetpinball/##g' AppDir/*.desktop
}
...
REPO="Spacecadetpinball-git-appimage"
TAG="latest"
